...
class RootWidget(GridLayout):
    ...
    def save_model(self):
        self.cnn.model.save_weights('rat_weights_24_boundary.h5',overwrite=True)
    ...
    def move_training(self, feature_vector):
      if(not self.isMoving):
        ...
        if((feature_vector[12]+feature_vector[13]+feature_vector[11])/3.0<=np.mean(feature_vector)):
          self.isMoving=True
      else:
    ...
    def move_predict(self,pred):
      if(not self.isMoving):
        self.motion.set_speed(0,self.motion_direction*0.4)
        if((pred[0,12]+pred[0,13]+pred[0,11])/3.0<=np.mean(pred[0])):
          self.isMoving=True
      else:
        self.motion.set_speed(1,0)
        if((pred[0,12]+pred[0,13]+pred[0,11])/3.0>np.mean(pred[0])):
    ...
    def start_test(self, *args):
      ...
      np_image_RGB = np.delete(np_image_RGBA,(3), axis=2)
      np_image_RGB= np_image_RGB/255.0
      self.cnn=CNNTraining(np_image_RGB,24)
      self.currentNode=0
      ...
      if(self.check_1.active):
        self.load_model('rat_weights_24_boundary.h5')
      # Remove the button.
