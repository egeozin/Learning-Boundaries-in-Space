...
class RootWidget(GridLayout):
    ...
    def save_model(self):
        self.cnn.model.save_weights('rat_weights_12_boundary_MSE.h5',overwrite=True)
    ...
    def move_predict(self,pred):
      if(not self.isMoving):
        self.motion.set_speed(0,self.motion_direction*0.4)
        if((pred[0,5]+pred[0,6]+pred[0,7])/3.0<=np.mean(pred[0])):
          self.isMoving=True
      else:
        self.motion.set_speed(1,0)
        if(pred[0,6]>np.mean(pred[0])):
          self.isMoving=False
          self.motion_direction=np.sign(random.random()-0.5)*self.motion_direction
    ...
    def start_test(self, *args):
      ...
      self.data_file=open('3Min_Training_12B_Room7_pred_MSE3','w')
      if(self.check_1.active):
        self.load_model('rat_weights_12_boundary_MSE.h5')
      # Remove the button.
