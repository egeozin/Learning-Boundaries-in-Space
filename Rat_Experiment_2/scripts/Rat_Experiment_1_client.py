...
class RootWidget(GridLayout):
    ...
    def save_model(self):
        self.cnn.model.save_weights('rat_weights_2.h5',overwrite=True)
    ...
    def updateData(self,dt):
          ...
          image = PILImage.frombytes('RGBA',(self.imageSize,self.imageSize),imageFormat)
          ...
          if(self.training):
            ...
            global counter 
            counter += 1
            loss = self.cnn.update_model_online_lr(np_image_RGB, feature_vector, counter)
            print(counter)
            self.move_training(feature_vector)
            if(self.record_mode):
              self.data_file.write("%s %s %s %s  \n" % (pos_x ,  pos_y , raw_direction ,  feature_vector))
          else:
            pred  = self.cnn.predict_model(np_image_RGB)
            print("Prediction : ",pred)
            print ("Loss", np.mean(np.square(np.log(np.maximum(0.000000001,pred)+1.)-np.log(np.maximum(0.000000001,feature_vector)+1.))))
            self.move_predict(pred)
            if(self.record_mode):
              self.data_file.write("%s %s %s %s \n" % (pos_x , pos_y , raw_direction , pred))
    ...
    def move_training(self, feature_vector):
      if(not self.isMoving):
        self.motion.set_speed(0,self.motion_direction*0.4)
        if((feature_vector[6]+feature_vector[7]+feature_vector[5])/3.0<=np.mean(feature_vector)):
          self.isMoving=True
      else:
    ...
    def move_predict(self,pred):
      if(not self.isMoving):
        self.motion.set_speed(0,self.motion_direction*0.4)
        if((pred[0,6]+pred[0,7]+pred[0,5])/3.0<=np.mean(pred[0])):
          self.isMoving=True
      else:
        self.motion.set_speed(1,0)
        if((pred[0,6]+pred[0,7]+pred[0,5])/3.0>np.mean(pred[0])):
    ...
    def start_test(self, *args):
      ...
      np_image_RGB = np.delete(np_image_RGBA,(3), axis=2)
      np_image_RGB= np_image_RGB/255.0
      self.cnn=CNNTraining(np_image_RGB,12)
      self.currentNode=0
      ...
      self.data_file=open('RecordSession_1','w')
      if(self.check_1.active):
        self.load_model('rat_weights.h5')
      # Remove the button.
